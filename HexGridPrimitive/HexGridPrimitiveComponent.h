...
public:
	...
	UFUNCTION(BlueprintCallable, Category = "地块管理")
	void ApplyTileTypeToMaterial(EHexTileType TileType);
	...
	// 设置可用地块类型
	UFUNCTION(BlueprintCallable, Category = "地块管理")
	void SetAvailableTileTypes(const TArray<EHexTileType>& NewTypes);
	...
	// 检查是否需要重新生成地块类型
	UFUNCTION(BlueprintPure, Category = "地块管理")
	bool ShouldRegenerateTileTypes() const;

	// 智能重新生成（只在需要时重新生成）
	UFUNCTION(BlueprintCallable, Category = "地块管理")
	void SmartRegenerateTiles();

	// 强制重新生成所有地块类型
	UFUNCTION(BlueprintCallable, Category = "地块管理")
	void ForceRegenerateAllTiles();

	// 重新生成随机地块（蓝图友好）
	UFUNCTION(BlueprintCallable, Category = "地块管理", CallInEditor)
	void RegenerateRandomTiles();

	// 设置草地和水类型
	UFUNCTION(BlueprintCallable, Category = "地块管理")
	void SetGrassAndWaterTypes();

	// 获取地块类型统计
	UFUNCTION(BlueprintPure, Category = "地块管理")
	TMap<EHexTileType, int32> GetTileTypeStatistics() const;
